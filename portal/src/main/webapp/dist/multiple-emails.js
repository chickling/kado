		...
		return this.each(function() {
			...
			if ($(this).val() != '' && IsJsonString($(this).val())) {
				$.each(jQuery.parseJSON($(this).val()), function( index, val ) {
					$list.append($('<label class="ui label multiple_emails-email"><span class="email_name" data-email="' + val.toLowerCase() + '">' + val + '</span></label>')
			...
			function display_email(t, dupEmailCheck) {
				...
				for	(var i = 0; i < arr.length; i++) {
					...
					if ( dupEmailCheck === true && $orig.val().indexOf(arr[i]) != -1 ) {
						...
						$list.append($('<label class="ui label multiple_emails-email"><span class="email_name" data-email="' + arr[i].toLowerCase() + '">' + arr[i] + '</span></label>')
							  .prepend($(deleteIconHTML)
								   .click(function(e) { $(this).parent().remove(); refresh_emails(); e.preventDefault(); })
							  )
						);
