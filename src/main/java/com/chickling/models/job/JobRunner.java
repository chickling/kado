...
//                    List<String> activeWriter=Arrays.asList(binarySaveType.split("0");
//                    List<String> activeWriter=Arrays.asList(binarySaveType.split("0");

//
//                    if (PrestoContent.HDFS.equals(job.getSave_type())) {
//                        //*************************************************************************************
//                        // Save Result with CSV format to User Define HDFS Path
//                        //*************************************************************************************
//                        log.info("Save CSV Result to HDFS");
//                        if (!saveResultToCSV(jobLog)){
//                            this.exception.append("saveResult to CSV Error").append("\n");
//                            isSuccess=Boolean.FALSE;
//                            return isSuccess;
//                        }
//                    } else if (PrestoContent.DATABASE.equals(job.getSave_type())){
//                        //*************************************************************************************
//                        // insert To SQL Server
//                        //*************************************************************************************
//                        String connName=Init.getLocationList().get(job.getLocation_id());
//                        log.info("Start Insert Result to SQL Server : [ "+connName+" ]");
//                        ImportDB importDB = new ImportDB(new String(Base64.getDecoder().decode(job.getInsertsql()), "UTF-8"), jobLog.getJoboutput(),connName, resultCount, Init.getImportBatchSize());
//                        importDB.execute();
//                        if (!importDB.isSuccess()){
//                            log.error("Import to DB  Error!! ");
//                            log.error(importDB.getException());
//                            this.exception.append(importDB.getException()).append("\n");
//                            isSuccess=Boolean.FALSE;
//                            return isSuccess;
//                        }
//                    }
...
//        String localLogPath=JobRunner.class.getResource("/")+ThreadContext.get("logFileName");
    ...
    private boolean doWriter(List<String> activeWriter , Map parameter){
        int resultCode=0;
//        try {
            //HDFS binary  100
            //
